...
let StyledNode = require('./styled-node');
let MatchedRule = require('./matched-rule');
...
function styleTree (root, stylesheet) {
    ...
    let values = root.nodeType === 1 ? specifiedValues(root, stylesheet) : {},
...
function specifiedValues (elem, stylesheet) {
    ...
    let rules = matchedRules.map(function (matchedRule) {
...
function matchRule (elem, rule) {
    let matchedSelector = _.find(rule.selectors, function (selector) {
